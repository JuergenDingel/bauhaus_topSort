...
# 'less than' on tasks is a total order (i.e., for each pair of different tasks, exactly one of them is less than the other)
# for task1 in TASK:
#     for task2 in TASK:
#         if task1!=task2:
#             constraint.add_exactly_one(E, DepOn(task1,task2), DepOn(task2,task1))
...
for t1 in TASK:
    for t2 in TASK:
        match (t1,t2):
            case (1,2): E.add_constraint(DepOn(1,2))    # t1 > t2
            case (2,3): E.add_constraint(DepOn(2,3))    # t1 > t2
            case _: E.add_constraint(~DepOn(t1,t2))

# E.add_constraint(DepOn(1,2))    # t3 > t2
# E.add_constraint(DepOn(2,3))    # t2 > t1
